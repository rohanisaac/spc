...
class subFileOld:
    """ 
    Processes each subfile passed to it, extracts header information and data
    information and places them in data members. Used for the old format where the 
    y-values are stored in an odd way
    ...
    def __init__(self, data, fnpts, fexp, txyxy):
        ...
        # for old format, extract the entire array out as 1 bit unsigned integers,
        # swap 1st and 2nd byte, as well as 3rd and 4th byte to get the final integer
        # then scale by the exponent
        if yfloat:
            y_dat_str = '<'+ 'f'*pts
        else:
            y_dat_str = '>' + 'b'*4*pts
